...
function parseCsvStruct(args: {
    ...
    if (!lines || lines.length < 2) {
        throw new Error(`Expected at least 2 CSV lines (header + content) ${args.args}`);
    }
    const rawHeader = lines[0];
    const header: string[] = rawHeader.split(CSV_SEPARATOR);
    if (!header || header.length < 2) {
        throw new Error(`Expected at least 2 columns in CSV header with separator '${CSV_SEPARATOR}' ${args.args}`);
    }
    const languageCodes = header.slice(1);
    const languageIndex =
      1 + languageCodes.findIndex((value) => value.trim() === args.args.lng);
    if (languageIndex <= 0) {
        throw new Error(`Did not find language '${args.args.lng}' in CSV header '${rawHeader}'`);
    }
...
export class SimpleCsv implements TFileFormat {
    readTFile(args: ReadTFileArgs): Promise<TSet> {
      ...
      csvStruct.contentLines.forEach((line) => {
        ...
        if (tSet.has(key)) {
            throw new Error(`duplicate key '${key}' -> Currently, the usage of duplicate translation-keys is discouraged.`);
        }
        tSet.set(key, value);
      });
